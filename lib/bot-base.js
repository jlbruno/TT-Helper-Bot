...
var botConfig = {
	...
	autobop   : false, // whether bot should autobop
	autobot   : false, // whether bot should speak "bot dance" to make other bots dance
	autoDj    : true, // whether bot should get on deck automatically if down to 1 DJ
	danceMode : true, // whether bot is allowed to dance at all
	danceVotesReq: 1, // votes needed to dance when requested
	followMe  : false, // if bot should follow the bot owner to a new room
	debug     : false, 
	holdMode  : false, // if true, bot will skip it's own song when DJing
	watchMode : false, // true to watch room activity through PM from another room
	...
	botRegister	: false, // to register the account as a bot with TT - should be true in your real bot config
	lastfm    : {
		api_key: "LASTFM_KEY",
...
var doCommand = function(commandObj) {
	...
	switch(commandObj.command){
		...
		case 'q':
			...
			botSpeak(where, returnQueueAsString());
			break;
		case 'q+':
		...
		case 'q-':
			...
			break;
		case 'hi':
			botSpeak(where, "hi", commandObj.pmID);
			break;
		case 'theme':
			// TODO return the theme
			botSpeak(where, ttRoom.theme, commandObj.pmID);
			break;
		case 'songlimit':
			botSpeak(where, config.songLimit, commandObj.pmID);
			break;
		case 'cook':
			botSpeak(where, "https://www.tiktok.com/@chloemaxlang/video/6813083449801362693", commandObj.pmID);
			break;
		...
		case 'timers':
			...
			break;
		case 'songcount':
			if (spkr.isDj || spkr.isMod || spkr.isOwner) botSpeak(where, getDJSongCount(), commandObj.pmID);
			break;
		case 'current':
		...
		case 'settheme':
			if (spkr.isMod || spkr.isOwner) {
				// TODO set theme
				ttRoom.theme = commandObj.paramOrig;
			}
			break;
		case 'setsonglimit':
			if (spkr.isMod || spkr.isOwner) {
				// TODO set theme
				config.songLimit = param;
			}
			break;
		...
		case 'goodnight':
		case 'goodbye':
				if (spkr.isOwner) bot.roomDeregister();
				break;
		case 'watch':
...
var connect = function(roomid) {
	...
	bot.roomRegister(roomid, function (data) {
		...
		// this line can register the bot as a bot with TT
		// it seems this would avoid the need for email verification
		// unsure what else it would affect at this point
		if (config.botRegister === true) {
...
var onRegistered = function (data) {
	var strModWelcome = "Welcome to the room. Current DJ idle times: " + getIdleTimes();
	var strWelcome = "Welcome to the room!";
	var isMod = isRoomMod(data.user[0].userid);

	if (config.welcomeMods) {
		if (isMod || data.user[0].userid == config.botOwner) {
			botSpeak('pm', strModWelcome, data.user[0].userid);
		}
	}
	if (config.welcomeAll) {
		//botSpeak('pm', strWelcome, data.user[0].userid);
	}
};
...
// update the count of how many songs each DJ has played
var updateDJSongCount = function(data) {

	// check for null, because this might fail if the deck is empty and then someone starts DJing
	if (data.room.metadata.current_song != null) {
		
		// if there's no song count, set it to 0
		var currentSongCount = ttRoom.djList[data.room.metadata.current_song.djid].songCount || 0;
		// then add 1
		ttRoom.djList[data.room.metadata.current_song.djid].songCount = currentSongCount + 1;
	}
	
}

// see if the DJ has hit the song limit
// figure out what to do if so
var checkDJSongLimit = function(data) {
	var songCount = ttRoom.djList[data.room.metadata.current_song.djid].songCount;
	
	console.log("check dj song limit");
	...
	if (songCount >= config.songLimit) {
		
		// is the deck full? 
		if (data.room.metadata.dj_full == true) {
			//is there anyone in queue? 
			if (botState.queue.length && botState.queue.length > 0) {
				// let them know they hit their limit
				botSpeak("chat", "@" + djname + " You've hit the song limit! Please consider stepping down so others can DJ.");	
			}
			
			// TODO kick them off deck if there's an active queue
		}
...
// return the list of DJs and the amount of songs they have played
var getDJSongCount = function() {
	
	var str = '';
	for (var dj in ttRoom.djList) {
		var djObj = ttRoom.djList[dj];
		var songCount = djObj.songCount || 0;
		
		str = str + djObj.name + ': ' + songCount + ';   ';
...
var returnQueueAsString = function() {
...
var baseBot = {
	...
	init : function(botObj) {
		bot = botObj.bot;
		config = {...botConfig, ...botObj.config};
		...
		bot.on('endsong', function (data) {
			updateDJSongCount(data);
			checkDJSongLimit(data);
		});
