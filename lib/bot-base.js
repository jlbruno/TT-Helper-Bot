...
var onRegistered = function (data) {
	var modWelcome = "Welcome to the room. Current DJ idle times: " + getIdleTimes();
	var welcome = "Welcome to the room!";
	var isMod = isRoomMod(data.user[0].userid);

	if (isMod || data.user[0].userid == config.botOwner) {
		botSpeak('pm', modWelcome, data.user[0].userid);
	} else {
		//botSpeak('pm', welcome, data.user[0].userid);
	}
};
